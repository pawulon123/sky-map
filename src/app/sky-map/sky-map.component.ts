...
interface Star { id?: number|string; ra?: number; ra_deg?: number; dec: number; mag?: number; name?: string; __projected?: [number, number]; }
...
export class SkyMapComponent implements OnInit {
  ...
  projectedStars = computed(() => {
    ...
    return (this.starsData().stars || []).map((s) => {
      ...
      const p = proj([-ra, dec]) as [number, number];
